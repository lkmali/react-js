...
class AdminTaskAddUsersComponent extends Component {
  ...
  componentDidMount() {
    this.getUserList().then((x) => this.getTaskData())
  ...
  navigateAddUser = () => {
    this.props.navigate(`/home/user-task/${this.props.params?.taskId}/add-users`, {
      fromMyComponent: true,
    })
  }
  getUserList = async (filter = this.state.filter) => {
    try {
      const headers = { Authorization: `Bearer ${storageActions.getItem('token')}` }
      this.setState({ loader: true, filter })
      let url = `${RestUrlHelper.GET_USER_LIST_URL}${getQueryParameter(filter)}`
      const res = await restActions.GET(url, { headers })
  ...
  onParentSelect = (e) => {
    ...
    if (e.target.checked) {
      for (const value of this.state.userData) {
  ...
  addUsers = async () => {
    try {
      ...
      const url = `${RestUrlHelper.PUT_Task_CREATE_URL}/${taskId}/users`
      this.setState({ addButtonFlag: true })
      let newSelectedUser = selectedUser.filter((item) =>
        alreadyAddedUser.find((x) => x.userId != item),
      )
      const putPromise = restActions.PUT(url, newSelectedUser)
      const deletePromise =
        deletedUser.length > 0
          ? restActions.DELETE(
              url,
              deletedUser.map(({ userTaskId }) => userTaskId),
            )
          : Promise.resolve()
      ...
      this.props.navigate(`/home/user-task/view/${taskId}`)
    } catch (err) {
      this.setState({ addButtonFlag: false })
      NotificationMessage.showError(err.message)
    }
  ...
  //   const url = `${RestUrlHelper.PUT_Task_CREATE_URL}/${taskId}/users`
  //   this.setState({ addButtonFlag: true })
  //   let newSelectedUser = selectedUser.filter((item) => alreadyAddedUser.find(x => x.userId != item))
  //   restActions.PUT(url, newSelectedUser).then(
  //     async () => {
    ...
    return (
      <div>
        ...
        <EditTaskUser
          ...
          data={this.props.screenType === 'view-users' ? this.state.userData : this.state.userData}
