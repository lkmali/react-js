...
import AdminTaskListContainer from '../../../container/admin/user-task/AdminTaskListContainer'
...
class AdminFormsComponent extends Component {
  ...
  getFormList = (filter) => {
    ...
    restActions
      .GET(
        `${RestUrlHelper.GET_Url_Task_URL}${getQueryParameter({
          ...filter,
          cacheKey: 'user-task',
  ...
  createTask = () => {
    ...
    this.props.navigate('/home/user-task/create')
  }
  ...
  handleDownloadXlsx = async () => {
    ...
    const data = await downloadXlsx({ userFromId }, 'task')
