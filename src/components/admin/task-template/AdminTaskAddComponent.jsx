...
import AdminTaskAddContainer from '../../../container/admin/task-template/AdminTaskAddContainer'
...
class AdminTaskAddComponent extends Component {
  ...
  getTaskDetails = async (taskId = this.state.taskId) => {
    try {
      this.setState({ taskFetchLoader: true })
      let taskDetails = await restActions.GET(
        `${RestUrlHelper.GET_Task_TEMPLATE_CREATE_URL}?id=${taskId}`,
      ...
      this.setState({ taskFetchLoader: false })
  ...
  getFormList = (projectId) => {
    ...
    restActions.GET(`${RestUrlHelper.GET_FORM_LIST_URL_BY_PROJECTID}`, { headers }).then(
      (response) => {
  ...
  handleStatus = async (taskId = this.state.taskId, status) => {
    try {
      this.setState({ taskStatusLoader: true })
      status = status ? 'inActive' : 'active'
      let response = ''
      response = await restActions.PATCH(
        `${RestUrlHelper.POST_Task_TEMPLATE_CREATE_URL}/${taskId}/publish`,
        getAuthorizationHeaders(),
      )
      if (response && response.status == 200) {
        NotificationMessage.showInfo(
          `Task ${status == 'isPublish' ? 'Not Published' : 'Published Successfully'} `,
        )
      }
  ...
  handleSubmit = async (requestObject) => {
    ...
    try {
      ...
      let response = ''
      if (requestObject && requestObject.projectFormId && requestObject.projectFormId.value) {
        requestObject.projectFormId = requestObject.projectFormId.value
      }
      if (isUpsert) {
        message = 'updation'
        response = await restActions.PUT(
          `${RestUrlHelper.POST_Task_TEMPLATE_CREATE_URL}/${this?.state?.taskId}`,
          requestObject,
          getAuthorizationHeaders(),
        )
      } else {
        ...
        response = await restActions.POST(
          RestUrlHelper.POST_Task_TEMPLATE_CREATE_URL,
          requestObject,
          getAuthorizationHeaders(),
        )
      }
      ...
      this.setState(
        ...
        () => {
          ...
          if (redirection) {
            this.props.navigate(`/home/task-template/view/${this?.state?.taskId}`)
    ...
    return (
      <>
        ...
        >
          ...
          {/* <div className='col-sm-6 border box-shadow rounded'>
            <p className='script'>
